<div align="center">
  ...
  [![Version](https://img.shields.io/badge/release-v0.7.1_stable-22C55E?style=for-the-badge)](https://github.com/legeling/PromptHub/releases/latest)
...
Latest stable: **v0.7.1**. Two download lanes for every platform:
...
> The shots below cover the current stable v0.7.1 surfaces.
...
- **Category Tag Management**: Supports organizing public skills into categories. Admins can easily assign and update categories from the admin dashboard via inline select dropdowns.
- **Namespace & Unique Identity**: Introduces a namespace system similar to npm. Public skills must use a unique `registry_slug` in the format `{owner_username}/{skill_name}`, avoiding conflicts and identity squatting on the public marketplace.
...
- Added the Agent Project Management System (Phase 1) supporting template creation/import, Python templates, and a three-column WebSocket chat UI.
- Upgraded Agent Session management, supporting timestamp naming, WebSocket auto-reconnect, and suggestions in the empty chat panel.
- Fixed Agent venv startup crash, introducing venv-only mode; resolved duplicate IPC handler registration crash on desktop startup, and appearance settings blank screen.
- Removed WebDAV and S3 sync providers, keeping only "Self-Hosted PromptHub" as the cloud sync target, clean up ~5800 lines of code.
- Supported User Data isolation by desktop sync account with automatic state reload on switch.
- Removed login/registration captcha for web setups, simplifying the bootstrap/setup flow, and added a header logout button.

### v0.6.0 (2026-06-19)

- Added SkillHub Admin Approval Workflow, transitioning Skill publishing to a review-based submission mechanism.
- Added a standalone Admin Panel (`/admin`) for statistics, pending skill reviews, user role management, and list management.
- Implemented user role promotion/demotion (admin/user) with operation guards (no self-deletion, no last-admin demotion).
- Supported admin panel dark theme, aligning with the existing SkillHub UI design.
- Fixed desktop startup FK constraint failures when inserting/importing skills, and desktop→web SkillHub mirror publishing pipeline.
- Synchronized locales to support 7 UI languages across the new review flows.
...
- Built-in Agent Project Management System (Phase 1) and Python nanobot Agent template
- Three-column real-time WebSocket chat UI with session auto-assign and auto-reconnect
- User data path isolation by account and automatic setting reloads on account switch
- Consolidated cloud sync by removing WebDAV and S3 in favor of Self-Hosted PromptHub sync

### v0.6.0

- SkillHub admin review workflow and standalone `/admin` dashboard
- Dashboard statistics, skill moderation, user role promotion/demotion, and safety guards
- Complete 7-locale translations and dark-mode styling for the admin panel
