...
// 快捷键配置存储路径
const getLegacyShortcutsPath = () =>
  path.join(app.getPath("userData"), "shortcuts.json");
const getLegacyShortcutModePath = () =>
  path.join(app.getPath("userData"), "shortcut-mode.json");
const getConfiguredShortcutsPath = () =>
  path.join(getConfigDir(), "shortcuts.json");
...
function registerSingleShortcut(action: string, accelerator: string): boolean {
  ...
  try {
    ...
    if (!success) {
      console.warn(
        `Failed to register shortcut: ${accelerator} for action: ${action}`,
      );
    }
    return success;
  } catch (error) {
...
export function registerShortcutsIPC(): void {
  ...
  ipcMain.on(
    "shortcuts:setMode",
    (_event, modes: Record<string, "global" | "local">) => {
      shortcutModes = modes;
