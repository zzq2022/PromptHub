...
export function useTableConfig() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (
          parsed.columnOrder &&
          parsed.columnOrder.length === DEFAULT_ORDER.length
        ) {
  ...
  const getVisibleColumns = useCallback(() => {
    ...
    const stickyRight = columns.filter(
      (c) => c.sticky === "right" && c.visible,
    );
    const reorderable = columnOrder
      .map((id) => columns.find((c) => c.id === id))
      .filter(
        (c): c is ColumnConfig => c !== undefined && c.visible && !c.sticky,
      );
