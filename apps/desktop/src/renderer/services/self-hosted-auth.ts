...
const CAPTCHA_PATH_SIGNATURE_TO_CHAR: Record<string, string> = {
  ...
  MLLQLLQLLQLLQLLQLLQLLQLLQLLQLLLQLLQLLQLLQLLQLLLQLLQLLQLLQZMLLQLLQLLQLLQLLQLLQLLQLLQLLQLLLQLLQLLQLLLLQLLQLLQLLQLLQLLQLLQLLQLLQLLQLLQLLQLLQLLQZ: "E",
...
function extractPromptHubCaptchaPathAttributes(svg: string): string[] {
  if (typeof DOMParser === "function") {
    ...
    return [...document.querySelectorAll("path")].map((path) => {
      ...
      return [fill ? `fill="${fill}"` : "", stroke ? `stroke="${stroke}"` : "", pathData ? `d="${pathData}"` : ""]
        .filter(Boolean)
        .join(" ");
    });
  }
...
function decodePromptHubCaptchaImageData(imageData: string): string {
  ...
  const glyphs = extractPromptHubCaptchaPathAttributes(svg)
    ...
    .filter((glyph): glyph is { x: number; signature: string } => glyph !== null)
...
export async function issueSolvedPromptHubCaptcha(baseUrl: string): Promise<{
  ...
  throw new Error("PromptHub captcha response did not include a solvable challenge");
}
