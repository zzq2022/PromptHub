...
export function CollapsibleThinking({
  ...
  useEffect(() => {
    if (
      ...
      contentRef.current &&
      content &&
      content.length > prevContentLength.current
    ) {
  ...
  return (
    <div
      className={`rounded-xl border border-border/50 bg-muted/20 overflow-hidden transition-all duration-base ${className}`}
    >
      {/* Header / 头部 */}
      <button
      ...
      >
        ...
        <span
          className={`relative ${isLoading ? "text-primary" : "text-muted-foreground"}`}
        >
          <BrainIcon
      ...
      {/* Content / 内容 */}
      <div
        className={`overflow-hidden transition-all duration-smooth ease-in-out ${
