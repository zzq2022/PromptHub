...
export function VariableInputModal({
  ...
  const readImageFileAsAttachment = (
    file: File,
  ...
  const handleImageSelection = async (files: FileList | null) => {
    ...
    const acceptedFiles = Array.from(files)
      ...
      .filter(
        (file) =>
          SUPPORTED_AI_TEST_IMAGE_MIME_TYPES.has(file.type) &&
          file.size <= MAX_AI_TEST_IMAGE_BYTES,
      );
    ...
    try {
      ...
      setImageAttachments((prev) =>
        [...prev, ...nextAttachments].slice(0, MAX_AI_TEST_IMAGES),
      );
    } catch {
