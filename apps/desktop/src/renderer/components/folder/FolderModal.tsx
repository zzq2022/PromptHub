...
export function FolderModal({ isOpen, onClose, folder }: FolderModalProps) {
  ...
  const [deleteMode, setDeleteMode] = useState<"folder-only" | "all-content">(
    "folder-only",
  );
    ...
    function traverse(nodes: FolderTreeNode[]) {
      nodes.forEach((node) => {
        ...
        const isValidParent =
          !isEditMode ||
          (folder?.id !== node.id &&
            canSetParent(folders, folder!.id, node.id));
  ...
  const executeDelete = async () => {
    ...
    try {
      if (deleteMode === "all-content") {
        ...
        await deleteFolder(folder.id);
        showToast(
          t("folder.deletedWithPrompts", { count: folderPrompts.length }),
          "success",
        );
      } else {
        ...
        await deleteFolder(folder.id);
        showToast(
          t("folder.deletedMovedPrompts", { count: folderPrompts.length }),
          "success",
        );
      }
      setShowDeleteOptions(false);
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <ChevronRightIcon
                      className={`w-4 h-4 text-muted-foreground transition-transform ${showParentSelect ? "rotate-90" : ""}`}
                    />
                  </button>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {t("folder.parentHint", "最多支持 {{depth}} 层嵌套", {
                  ...
                  <div className="pl-6 animate-in fade-in slide-in-from-top-2 duration-base">
                    {!securityStatus.configured ? (
                      <p className="text-xs text-destructive">
                        {t(
                          "folder.privateNeedPasswordInline",
                          '请到"设置 - 安全"设置主密码后再开启私密。',
                        )}
                      </p>
                    ) : (
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center">
          ...
          <div className="relative app-wallpaper-panel-strong rounded-xl w-full max-w-sm mx-4 p-5 border border-border space-y-4">
            <h3 className="text-base font-semibold">
              {t("folder.unlockTitle", "输入主密码")}
            </h3>
            <p className="text-xs text-muted-foreground">
              {t("folder.unlockDesc", "保存私密文件夹前需要先解锁主密码")}
            </p>
            <input
              type="password"
            ...
            <div className="flex gap-2 justify-end">
              ...
              >
                {unlocking
                  ? t("folder.unlocking", "解锁中...")
                  : t("settings.unlock", "解锁")}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="relative app-wallpaper-panel-strong rounded-xl w-full max-w-md mx-4 p-5 border border-border space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-base font-semibold">
                  {t("folder.deleteTitle", "删除文件夹「{{name}}」", {
                    name: folder?.name || "",
                  })}
                </h3>
                <p className="text-sm text-muted-foreground mt-1">
                  {t(
                    "folder.containsPrompts",
                    "此文件夹包含 {{count}} 个提示词",
                    { count: promptsInFolder },
                  )}
                </p>
              </div>
            </div>
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <div className="text-sm font-medium">
                    {t("folder.deleteFolderOnly", "仅删除文件夹")}
                  </div>
                  <div className="text-xs text-muted-foreground mt-0.5">
                    {t(
                      "folder.deleteFolderOnlyDesc",
                      "保留 {{count}} 个提示词，移至根目录",
                      { count: promptsInFolder },
                    )}
                  </div>
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <div className="text-sm font-medium text-destructive">
                    {t("folder.deleteAllContent", "删除所有内容")}
                  </div>
                  <div className="text-xs text-muted-foreground mt-0.5">
                    {t(
                      "folder.deleteAllContentDesc",
                      "删除文件夹及内部所有 {{count}} 个提示词",
                      { count: promptsInFolder },
                    )}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center">
          ...
          <div className="relative app-wallpaper-panel-strong rounded-xl w-full max-w-sm mx-4 p-5 border border-border space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-base font-semibold">
                  {t("folder.duplicateTitle", "文件夹名称已存在")}
                </h3>
                <p className="text-sm text-muted-foreground mt-1">
                  {t(
                    "folder.duplicateDesc",
                    "已存在名为「{{name}}」的文件夹，是否仍要创建？",
                    { name: name.trim() },
                  )}
                </p>
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center">
          ...
          <div className="relative app-wallpaper-panel-strong rounded-xl w-full max-w-sm mx-4 p-5 border border-border space-y-4">
            <h3 className="text-base font-semibold text-destructive">
              {t("folder.deletePrivateTitle", "删除私密文件夹")}
            </h3>
            <p className="text-xs text-muted-foreground">
              {t(
                "folder.deletePrivateDesc",
                "此操作将删除文件夹「{{name}}」及其内的所有加密内容，请输入主密码确认",
                { name: folder?.name || "" },
              )}
            </p>
            <input
