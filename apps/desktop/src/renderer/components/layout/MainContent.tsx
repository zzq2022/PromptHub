...
const SkillManager = lazy(() =>
  import("../skill/SkillManager").then((m) => ({ default: m.SkillManager })),
);
...
const PromptQuickRewriteDialog = lazy(() =>
  import("../prompt/PromptQuickRewriteDialog").then((m) => ({
    default: m.PromptQuickRewriteDialog,
  })),
);
...
const PromptListView = lazy(() =>
  import("../prompt/PromptListView").then((m) => ({
    default: m.PromptListView,
  })),
);
const AiTestModal = lazy(() =>
  import("../prompt/AiTestModal").then((m) => ({ default: m.AiTestModal })),
);
const PromptDetailModal = lazy(() =>
  import("../prompt/PromptDetailModal").then((m) => ({
    default: m.PromptDetailModal,
  })),
);
const VariableInputModal = lazy(() =>
  import("../prompt/VariableInputModal").then((m) => ({
    default: m.VariableInputModal,
  })),
);
const VersionHistoryModal = lazy(() =>
  import("../prompt/VersionHistoryModal").then((m) => ({
...
function getHighlightTerms(searchQuery: string): string[] {
  ...
  if (compact && compact.length <= 64 && !terms.includes(compact))
    terms.push(compact);
...
const PromptCard = memo(function PromptCard({
  ...
  return (
    ...
    >
      ...
      {prompt.description && (
        <p
          className={`text-xs line-clamp-2 break-words mt-0.5 ${
...
function PromptSkillMainContent() {
  ...
  const setRenderMarkdownPref = useSettingsStore(
    (state) => state.setRenderMarkdown,
  );
  const [renderMarkdownEnabled, setRenderMarkdownEnabled] =
    useState(renderMarkdownPref);
  ...
  const handleSelectPrompt = useCallback(
    (prompt: Prompt, e: React.MouseEvent) => {
  ...
  const [promptTestStates, setPromptTestStates] = useState<
    Record<
      string,
      {
        isTestingAI: boolean;
  ...
  const aiResponse = isStreaming
    ? streamingContent
    : currentState?.aiResponse || null;
  const aiThinking = isStreaming
    ? streamingThinking
  ...
  const setIsComparingModels = (comparing: boolean) => {
    if (selectedId)
      updatePromptState(selectedId, { isComparingModels: comparing });
  };

  const setAiResponse = (
    response: string | null | ((prev: string | null) => string | null),
  ) => {
    if (selectedId) {
  ...
  const setAiThinking = (
    thinking: string | null | ((prev: string | null) => string | null),
  ) => {
    if (selectedId) {
  ...
  const setCompareResults = (
    results:
      | AITestResult[]
      | null
      | ((prev: AITestResult[] | null) => AITestResult[] | null),
  ) => {
    if (selectedId) {
      if (typeof results === "function") {
        const currentValue =
          promptTestStates[selectedId]?.compareResults || null;
        updatePromptState(selectedId, {
          compareResults: results(currentValue),
        });
      } else {
  ...
  const compareModels = useMemo(() => {
    const isImagePrompt =
      prompts.find((p) => p.id === selectedId)?.promptType === "image";
    if (isImagePrompt) {
      return [];
    }
  ...
  const canRunSingleAiTest = !!(
    (singleChatConfig.apiKey &&
      singleChatConfig.apiUrl &&
      singleChatConfig.model) ||
    (defaultImageModel &&
      defaultImageModel.apiKey &&
      defaultImageModel.apiUrl &&
  ...
  const readInlineAiTestImage = (
    file: File,
  ): Promise<VariableInputImageAttachment> => {
  ...
  const handleInlineAiTestImageSelection = async (files: FileList | null) => {
      ...
      if (file.size > MAX_AI_TEST_IMAGE_BYTES) {
        showToast(
          t("prompt.aiTestImageTooLarge", {
            name: file.name,
            size: formatAiTestImageSize(MAX_AI_TEST_IMAGE_BYTES),
          }),
          "error",
        );
        continue;
    ...
    try {
      const attachments = await Promise.all(
        acceptedFiles.map(readInlineAiTestImage),
      );
      setInlineAiTestImages((prev) =>
    ...
    try {
      ...
      if (currentPromptType === "image") {
        if (!defaultImageModel) {
          throw new Error(
            t("prompt.mismatchImage") ||
              "Prompt type is Image but no Image Model configured",
          );
        }

        console.log(
          "[MainContent] Image Prompt. Using model:",
          defaultImageModel.name || defaultImageModel.model,
        );
        try {
          ...
          if (imageUrl || imageBase64) {
            ...
            if (targetId) {
              try {
                ...
                if (imageUrl) {
                  // Download from URL
                  savedFileName = await (window.electron as any).downloadImage(
                    imageUrl,
                  );
                } else if (imageBase64) {
                  ...
                  ).saveImageBase64(fileName, imageBase64);
                ...
                console.warn(
                  "[MainContent] Failed to save generated image:",
                  saveErr,
                );
          ...
          setAiResponse(
            `${t("common.error")}: ${e instanceof Error ? e.message : "Image generation failed"}`,
          );
  ...
  useEffect(() => {
    ...
    const canRestore = visiblePrompts.some(
      (prompt) => prompt.id === lastSelectedId,
    );
    if (canRestore) {
  ...
  const [detailInlineDraft, setDetailInlineDraft] =
    useState<DetailInlineEditDraft>({
      title: "",
      description: "",
      systemPrompt: "",
      userPrompt: "",
  ...
  const detailInlineUserPromptField = useMemo<
    "userPrompt" | "userPromptEn"
  >(() => {
    if (!selectedPrompt) {
      return "userPrompt";
  ...
  const detailInlineSystemPromptField = useMemo<
    "systemPrompt" | "systemPromptEn"
  >(() => {
    if (!selectedPrompt) {
      return "systemPrompt";
  ...
  const hasDetailInlineChanges = useMemo(() => {
    ...
    return (
      detailInlineDraft.title.trim() !==
        detailInlineCurrentValues.title.trim() ||
      detailInlineDraft.description.trim() !==
        detailInlineCurrentValues.description.trim() ||
      detailInlineDraft.systemPrompt !==
        detailInlineCurrentValues.systemPrompt ||
  ...
  const handleDuplicatePrompt = useCallback(
    async (prompt: Prompt) => {
      ...
      showToast(
        t("prompt.promptDuplicated", "Prompt duplicate created"),
        "success",
      );
    },
  ...
  const handleDetailAddTag = useCallback(
    async (rawTag: string) => {
      ...
      if (
        !selectedPrompt ||
        !normalizedTag ||
        selectedPrompt.tags.includes(normalizedTag)
      ) {
        return;
      }
  ...
  const handleDetailTagDrop = useCallback(
    (event: ReactDragEvent<HTMLDivElement>) => {
      ...
      const droppedTag = event.dataTransfer.getData(
  ...
  const menuItems: ContextMenuItem[] = useMemo(() => {
    ...
    return [
      ...
      {
        label: contextMenu.prompt.isFavorite
          ? t("prompt.removeFromFavorites") || "取消收藏"
          : t("prompt.addToFavorites") || "收藏",
        icon: (
          <StarIcon
            className={`w-4 h-4 ${contextMenu.prompt.isFavorite ? "fill-yellow-400 text-yellow-400" : ""}`}
          />
        ),
        onClick: () => toggleFavorite(contextMenu.prompt.id),
      },
      {
        label: contextMenu.prompt.isPinned
          ? t("prompt.unpin")
          : t("prompt.pin"),
        icon: (
          <PinIcon
            className={`w-4 h-4 ${contextMenu.prompt.isPinned ? "fill-primary text-primary" : ""}`}
          />
        ),
  ...
  const getViewClass = useCallback(
    (mode: ViewMode, layout: "col" | "row" = "col") => {
                ...
                >
                  <div className="flex-1 overflow-y-auto">
                    ...
                    `}
                            >
                              <StarIcon
                                className={`w-5 h-5 ${selectedPrompt.isFavorite ? "fill-current" : ""}`}
                              />
                            </button>
                            <PromptQuickRewriteTrigger
                              onClick={() =>
                                setQuickRewritePrompt(selectedPrompt)
                              }
                        ...
                        <span
                          className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium shadow-sm ${
                            (selectedPrompt.promptType || "text") === "image"
                              ...
                              : (selectedPrompt.promptType || "text") ===
                                  "video"
                      ...
                      <div className="mb-4">
                        <div
                          ...
                          className={`flex min-h-[2.75rem] flex-wrap items-center gap-1.5 rounded-xl border py-1.5 pr-1.5 transition-[background-color,border-color,box-shadow] ${
                            isTagDropActive
                        ...
                        >
                          {selectedPrompt.tags.map((tag) => {
                            ...
                            return (
                              <span
                                key={tag}
                                className={`inline-flex max-w-full items-center rounded-full text-xs font-medium transition-colors ${
                                  isTagFiltered
                                    ? "bg-primary text-white"
                              ...
                              >
                                <button
                                  ...
                                  className={`inline-flex min-w-0 items-center gap-1 rounded-l-full px-3 py-1.5 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30 ${
                                    isTagFiltered
                                      ? "hover:bg-primary/90"
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    void handleDetailRemoveTag(tag)
                                  }
                                  title={t("prompt.removeTag", "Remove tag")}
                                  aria-label={`${t("prompt.removeTag", "Remove tag")}: ${tag}`}
                                  className={`inline-flex items-center justify-center rounded-r-full py-1.5 pl-1 pr-2 transition-colors focus-visible:outline-none focus-visible:ring-2 ${
                                    isTagFiltered
                                ...
                                const handled = handleMarkdownListKeyDown(
                                  ...
                                  (newValue, cursorPos) => {
                                    ...
                                    requestAnimationFrame(() => {
                                      detailSystemPromptTextareaRef.current?.setSelectionRange(
                                        cursorPos,
                                        cursorPos,
                                      );
                                    });
                            ...
                            onKeyDown={(event) => {
                              const handled = handleMarkdownListKeyDown(
                                ...
                                (newValue, cursorPos) => {
                                  ...
                                  requestAnimationFrame(() => {
                                    detailUserPromptTextareaRef.current?.setSelectionRange(
                                      cursorPos,
                                      cursorPos,
                                    );
                                  });
                      ...
                      {/* 多模型对比区域 */}
                      {selectedPrompt.promptType !== "image" &&
                                      ...
                                      <button
                                        onClick={async () => {
                                          try {
                                            ...
                                            if (
                                              !aiResponse.startsWith("data:")
                                            ) {
                                              try {
                                                const resp =
                                                  await fetch(aiResponse);
                                                const blob = await resp.blob();
                                                href =
                                                  URL.createObjectURL(blob);
                                              } catch (e) {
                                                console.warn(
                                                  "Failed to fetch image blob, falling back to direct link",
                                                  e,
                                                );
                                              }
                                            }
                    ...
                    <div className="w-full flex items-center gap-3 flex-wrap">
                      <button
                        onClick={async () => {
                          ...
                          // 根据语言模式选择内容
                          const currentUserPrompt = showEnglish
                            ? selectedPrompt.userPromptEn ||
                              selectedPrompt.userPrompt
                            : selectedPrompt.userPrompt;
                          const currentSystemPrompt = showEnglish
                            ? selectedPrompt.systemPromptEn ||
                              selectedPrompt.systemPrompt
                            : selectedPrompt.systemPrompt;
          ...
          {selectedPrompt && (
            <Suspense fallback={null}>
              <VariableInputModal
                ...
                userPrompt={
                  showEnglish
                    ? selectedPrompt.userPromptEn || selectedPrompt.userPrompt
                    : selectedPrompt.userPrompt
                }
                mode="copy"
                onCopy={async (text) => {
          ...
          {copyPrompt && (
            <Suspense fallback={null}>
              <VariableInputModal
                ...
                systemPrompt={
                  resolvePromptContentByLanguage(copyPrompt, showEnglish)
                    .systemPrompt
                }
                userPrompt={
                  resolvePromptContentByLanguage(copyPrompt, showEnglish)
